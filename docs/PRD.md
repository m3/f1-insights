...
## 🏎️ F1 Insights & Explanation Platform (v2026.20)
...
The platform unifies disparate telemetry feeds (FastF1, Ergast/Jolpica API, TracingInsights GitHub archives, OpenF1 API & WebSockets) and media radar signals (X/Twitter, YouTube watchalongs) into a single, high-reliability modular monolith powered by FastAPI, SQLite (WAL mode), and a React Carbon Dark dashboard.
...
### 4.1. Strategic Position Index Live Forecaster
* **FR-0.10 Live Outcome & SC Forecaster**: The system MUST implement `StrategyForecaster` to extrapolate green-flag lap time degradation decay and calculate Safety Car pit window time savings ($21.5\text{s} \rightarrow 11.2\text{s}$ loss) to answer questions **STRAT-04** and **STRAT-05**.

### 4.2. OpenF1 Real-Time WebSocket Streamer
* **FR-0.9 OpenF1 Live WebSocket Provider**: The system MUST implement `OpenF1WebSocketStreamer` to establish sub-second streaming connections (`wss://api.openf1.org/v1/live`) for live track positions with event-listener callbacks and exponential backoff reconnection.

### 4.3. Interactive AI Fan Question Cards
...
### 4.7. OpenF1 Real-Time Data Provider
...
### 4.8. Hidden Pace Detector Engine
...
### 4.9. Strategic Position Index (SPI) Engine
...
### 4.10. Domain Graph Traversals & Strongly-Typed Models
...
### 4.11. 20-Driver Grid & Session Classification Table
...
### 4.13. FIA Penalty Watch & Race Control
...
### 4.15. Model Context Protocol (MCP) Integration
